...
class RecommendationService:
    ...
    def recommend_meditations_for_user(user: UserInfo) -> List[Meditation]:
        ...
        meditation_theme_to_amount_of_meditations = RecommendationService.analyze_user_grades(user=user)
        ...
        for (meditation_theme, amount_of_meditations) in meditation_theme_to_amount_of_meditations:
            recommended_meditations += MeditationService.get_meditations_by_meditation_theme(
    ...
    def analyze_user_grades(user: UserInfo) -> List[tuple] | None:
        ...
        for user_grade in user_grades:
            ...
            user_theme_preference.add_grade(meditation_theme=meditation_theme, grade=user_grade.grade)

        logger.info(f"User total grades: {user_theme_preference.total_grades}")
        if user_theme_preference.total_grades < 5:
            return None

        meditation_theme_grade = RecommendationService.get_result_of_analysis(user_theme_preference)

        # Фильтруем темы с рейтингом ниже 4
        meditation_theme_grade_filtered = [(theme, rating) for theme, rating in meditation_theme_grade if rating >= 4.0]

        # Вычисляем пропорциональное количество медитаций для каждой темы
        meditations_per_theme = [floor(
            AMOUNT_OF_MEDITATIONS_TO_RECOMMEND * rating / sum(rating for _, rating in meditation_theme_grade_filtered))
        ...
        logger.info(f"meditation_name_to_amount_of_meditations: {meditation_name_to_amount_of_meditations}")
        return meditation_name_to_amount_of_meditations

    @staticmethod
    def get_result_of_analysis(user_theme_preference: UserThemePreference) -> List[tuple]:
        meditation_theme_grade = []
        for theme in user_theme_preference.theme_to_grades.keys():
            meditation_theme_grade.append((theme,
